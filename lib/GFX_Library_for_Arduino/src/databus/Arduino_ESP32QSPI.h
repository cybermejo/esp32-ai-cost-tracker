...
public:
  ...
  // FLUIDCUBE PATCH: source framebuffer is pre-swapped (big-endian RGB565),
  // so writePixels() can memcpy chunks instead of byte-swapping every pixel
  // pair on the CPU (the swap loop was stalling the SPI stream for ~10 ms).
  void setNoSwap(bool no_swap) { _no_swap = no_swap; }
...
private:
  ...
  bool _no_swap = false; // FLUIDCUBE PATCH
